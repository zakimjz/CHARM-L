...
void MaximalTest::addnode(vector<int> &prefix, int lit, 
                          LatticeNode *ln, int sup)
{
   unsigned int i,j;
   bool found;
...
bool MaximalTest::max_union(int sup, idlist *l1, idlist *&res){
   ...
   //return true if intersection not null
   //if (sup - res->size() <= 0) return false;
   //else return true;
   return true;
}
...
void MaximalTest::adjustlattice(LatticeNode *current,
                   idlist &supersets)
{
   ...
   for (int i=0; i < supersets.size(); ++i){
      //cout << "\t\tSUP: " << csuplist[supersets[i]] << " ";
      //cout << "NODE: " << *(lattice_nodes[supersets[i]]) << endl;
      //find the minimal supersets (use marker to remove non-minimal)
      lattice_nodes[supersets[i]]->mark_children(marker);
   }
   for (int i=0; i < supersets.size(); ++i){
      if (lattice_nodes[supersets[i]]->flag() != marker){
         //cout << "\t\tMIN: " << csuplist[supersets[i]] << " ";
         //cout << "NODE: " << *(lattice_nodes[supersets[i]]) << endl;
         //add the minimal superset as a child of currentnode
         current->add_child(lattice_nodes[supersets[i]]);
         
         //compare current with parent of lattice_node
         //if current is superset, remove lattice node (child) from parent
         //and also remove parent from lattice node
         list<LatticeNode *>::iterator par, tmp;
         for (par = lattice_nodes[supersets[i]]->parent()->begin();
              par != lattice_nodes[supersets[i]]->parent()->end();){
            if (current->subset(*par) == SUPSET){
               //cout << "\t\tPAR: " << *(*par) << endl;
               (*par)->remove_child(lattice_nodes[supersets[i]]);
               //cout << "\t\terase: "  << endl;
               tmp = par;
               ++par;
               lattice_nodes[supersets[i]]->remove_parent(*tmp);
            }
            else ++par;
         }
      }
