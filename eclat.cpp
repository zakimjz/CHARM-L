...
//global vars
char infile[300];
char consfile[300]; //constraint file
...
extern LatticeNode *form_closed_f2_lists(Eqclass *eq, LatticeNode *lattice);
extern void form_f2_lists(Eqclass *eq);
extern void generate_rules(LatticeNode *, vector<LatticeNode *> &);
...
void parse_args(int argc, char **argv)
{
   ...
   if (argc < 2){
      ...
      cout << "\t-C<constraints file>\n";
      cout << "\t-z<sorttype> (0=nosort, <1=incr>, 2=decr, 3=incr_noclass)\n";
      cout << "\t-o <outfile (if COUT, then stdout is used)>\n";
      cout << "\t-l (output + output_tidlist?)\n";
      cout << "\t-L (output lattice?)\n";
      cout << "\t-M<mingentype> (<0=minhitset>, 1=apriori) \n";
      cout << "\t-n<stop after mingen phase>\n";
      cout << "\t-b (binary input file?) (default=false)\n";
      cout << "\t-x (no offset (non-ibm-format?) (default=false/ibm)\n";
      cout << "\t-r <minconf> (generate rules? at what conf level?)\n";
      cout << "\t-R (print rules?)\n";
      cout << "\t-E (self rules only)\n";
      cout << "\t-F (conditional self rules also)\n";
      cout << "\t-Z <maxlen>\n";
      exit(-1);
   }
   else{
     while ((c=getopt(argc,argv,"a:bc:C:d:EFi:lLM:m:no:p:s:S:xz:r:RZ:"))!=-1){
       switch(c){
       case 'a':
	 ...
	 break;
       ...
       case 'L': 
          ...
          break;
       case 'M':
          mingen_type = (mingen_vals) atoi(optarg);
          break;
       case 'n':
          mingen_only = true;
          break;
	 ...
	 output = true;
         if (strcmp(optarg,"COUT") != 0){
            outfile.open(optarg);
         }
         else{
            outfile.copyfmt(std::cout);                                  //1
            outfile.clear(std::cout.rdstate());                          //2
            outfile.basic_ios<char>::rdbuf(std::cout.rdbuf());           //3
         }
       ...
       case 'R':
          ...
          break;
       case 'E':
          self_rules_only = true;
          break;
       case 'F':
          conditional_self_rules = true;
          break;
	 ...
	 break;
       ...
       case 'z':
          ...
          break;
       case 'Z':
          max_closed_len = atoi(optarg);
          break;
       }               
     }
   }

   if (output_idlist && !output){
      cerr << "must specify the -o flag with -l\n";
      exit(1);
   }
   
   //cout << "ENUM VALS " << alg_type << " " << diff_type << " " 
   //     << max_diff_type << " " << closed_type << " " << prune_type
   //     << endl;
   //cout << "INT MAX " << max_closed_len << endl;
}
...
void get_Fk(list<Eqclass *> &F2list){
   ...
   while(!F2list.empty()){
      ...
      case eclat:
         ...
         break;
      case charm:
         LatticeNode *ln = form_closed_f2_lists(eq, &lattice);
         newmax.clear();
         if (!ln) ln = &lattice;
         enumerate_closed_freq(eq, iter+1, newmax, ln);
         break;
//       case basicmax:
//          form_f2_lists(eq);
//          newmax.clear();
...
int main(int argc, char **argv)
{
   ...
   if (check_constraints) 
      constraints = new Constraints(consfile);
   ...
   if (rulegen || mingen_only) 
      generate_rules(&lattice, maxtest.lattice());
   
   if (output_lattice){
      for(int i=0; i < maxtest.lattice().size(); ++i){
         (maxtest.lattice())[i]->print_node(outfile,true);
      }
   }
   ...
   cout << "LATTICE = " << lattice_time << endl;
   cout << "MINGEN = " << mingen_time << endl;
   cout << "RULEGEN = " << rulegen_time << endl;
